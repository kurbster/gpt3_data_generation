...
def get_original_datasets(data_args: DataTrainingArguments, model_args: ModelArguments) -> dataset_dict:
    ...
    temp_dataset = datasets['train'].train_test_split(
        test_size=data_args.eval_split_percent, seed=data_args.random_seed
    )

    datasets['train'] = temp_dataset['train']
    datasets['validation'] = temp_dataset['test']
    del temp_dataset
...
def main(cfg):
    ...
    if len(data_args.train_files) > 0:
        for train_file in data_args.train_files:
            ...
            datasets["train"], datasets["validation"] = get_generated_dataset(train_file, data_args, model_args)
